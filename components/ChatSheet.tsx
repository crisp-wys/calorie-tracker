...
export default function ChatSheet({ open, onClose, prefill }: Props) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div
        className="fixed bottom-0 left-0 right-0 z-[110] h-[85vh] rounded-t-2xl bg-white flex flex-col shadow-[0_-4px_24px_rgba(0,0,0,0.12)]"
        style={{
          animation: closing
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-sm">{config.name || 'AI 教练'}</span>
          </div>
          ...
          <div className="relative bg-white rounded-[20px] p-6 w-full max-w-sm max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
                ...
                <button
                  ...
                  className={`rounded-xl p-1.5 transition-all ${
                    resolveAvatarId(config.avatar) === opt.id
                      ? 'ring-2 ring-brand ring-offset-2 bg-brand/10'
                      : 'hover:bg-gray-100'
                  }`}
                >
              ...
              <button
                ...
                className={`rounded-xl p-1.5 transition-all flex items-center justify-center border-2 border-dashed border-gray-300 hover:border-brand hover:text-brand ${
            ...
            {isCustomAvatar(config.avatar) && (
              <button
                ...
                className="text-xs text-gray-400 hover:text-red-500 transition-colors mb-4"
              >
                移除自定义头像
              </button>
            )}
